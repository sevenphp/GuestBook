<?php
	...
	if(isset($_COOKIE['username'])){
		...
		if(isset($rs['tc_uniqid'])){
			...
			$sql = "select tc_id from tc_message where tc_to_user='{$_COOKIE['username']}'";
			$query = mysql_query($sql);
			...
			$sql = "select `tc_id`,`tc_state`,`tc_from_user`,`tc_content`,`tc_sent_time` from `tc_message` where `tc_to_user`='{$_COOKIE['username']}' order by tc_sent_time desc limit $pag,$pagelimit";
			$query = mysql_query($sql);		
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title><?php echo $GLOBALS['sys_title'];?>--消息列表</title>
				...
				<form action="message_max_del.php" method="post" >
					<table border="1" cellspacing="0">
						...
						<tr>
							...
							<td class="content"><a href="message_detail.php?messid=<?php echo $rs['tc_id']; ?>" ><?php echo iconv_substr($content, 0,10,'utf-8').'...'; ?></a></td>
							<td><?php echo $rs['tc_sent_time']; ?></td>
							<td>
								<?php
									echo $img;
								?>
						...
						?>
						<tr><td colspan="5" class="maxdel"><label>全选<input type="checkbox" name="checkall" id="chkall" /></label>&nbsp;&nbsp;<input type="submit" name="maxdel" value="批量删除" /></td></tr>
					</table>
				</form>
				<?php
					pageList('message_list.php','?',$pagenum,$page);
				?>
			</div>
			<div class="clear"></div>
		</div>
